...
import { LayoutDashboard, FileText, UserCircle, LogOut, Bell, Menu, X, Sparkles } from "lucide-react";
...
export default function Layout({ user, setUser }: { user: any, setUser: any }) {
  ...
  const menuItems = [
    ...
    { name: "Career Consultant", path: "/consult", icon: Sparkles },
  ];
    ...
    <>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-4 h-4 shrink-0 ${isActive ? "text-cyan-400" : "text-[#E0E0E0]"}`} />
      ...
      <div className="p-6 shrink-0">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors opacity-60 hover:opacity-100 cursor-pointer rounded-lg text-left"
        >
  ...
  return (
    <div className="flex h-screen w-full bg-[#050505] text-[#E0E0E0] font-sans overflow-hidden selection:bg-cyan-500/30">
      ...
      {/* Desktop Sidebar */}
      <aside className="w-64 border-r border-white/10 bg-[#0A0A0A] flex-col z-20 hidden md:flex shrink-0">
        <SidebarContent />
      </aside>
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 left-0 h-full w-72 bg-[#0A0A0A] border-r border-white/10 z-50 flex flex-col md:hidden"
            >
              <button
                onClick={() => setMobileMenuOpen(false)}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative z-0">
        ...
        {/* Top Header */}
        <header className="h-16 md:h-20 border-b border-white/5 px-4 md:px-8 flex items-center justify-between shrink-0 relative z-[100] bg-[#050505]/80 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div className="md:hidden">
              <h2 className="text-base font-semibold text-white">
                {menuItems.find(m => location.pathname.startsWith(m.path))?.name ?? "CareerPilot"}
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <div onClick={handleReadNotifications} className="cursor-pointer p-1">
                {unreadCount > 0 && (
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute right-0 mt-4 w-72 md:w-80 bg-[#121212] border border-white/10 rounded-2xl shadow-xl overflow-hidden z-[200] p-2"
                  >
            ...
            <Link to="/profile" className="flex items-center gap-2 bg-white/5 py-1.5 px-3 rounded-full border border-white/10 cursor-pointer hover:bg-white/10 transition-colors">
              <div className="w-7 h-7 rounded-full bg-indigo-500 flex items-center justify-center font-bold text-white text-xs shrink-0">
        ...
        {/* Scrollable Content */}
        <div className="layout-scroll flex-1 overflow-y-auto w-full relative z-0 px-4 py-4 md:px-8 md:py-8">
        ...
        {showLogoutConfirm && (
          ...
          >
            <motion.div
              ...
              className="bg-[#121212] border border-white/10 p-6 rounded-2xl shadow-2xl max-w-sm w-full flex flex-col"
            >
              <h3 className="text-xl font-bold text-white mb-2">Sign out</h3>
              <p className="text-slate-400 mb-6">Are you sure you want to sign out of your account?</p>
              <div className="flex gap-3 justify-end">
                <button onClick={() => setShowLogoutConfirm(false)} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-white font-medium transition-colors">Cancel</button>
                <button onClick={confirmLogout} className="px-4 py-2 bg-red-500/80 hover:bg-red-500 rounded-xl text-white font-medium transition-colors">Sign Out</button>
              </div>
            </motion.div>
          </motion.div>
        )}
